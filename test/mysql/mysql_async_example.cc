 ...
 *
 * g++ -o async mysql_async_example.cc -I/usr/local/include/mariadb -L/usr/local/lib/mariadb -lmariadb -L/usr/local/lib -lev
 *
 */ 
...
static int opt_connections= 1;
static int opt_query_num = 10000;
#if 0
static const char *opt_db= "test";
static const char *opt_user= "ec";
static const char *opt_password= "ecEC!)@(#*$*";
static const char *opt_host= "192.168.1.222";
static const int opt_port= 3306; 
#endif
static const char *opt_db= "test";
static const char *opt_user= "ecuser";
static const char *opt_password= "ecuser";
static const char *opt_host= "10.0.201.141";
static const int opt_port= 3307; 
...
static void state_machine_handler(int fd , short event, void *arg) {
  ...
  while ( state_machine_continue){
    switch(conn->current_state) {
      ...
      case CONNECT_START:   
        status= mysql_real_connect_start(&conn->ret, &conn->mysql, opt_host, opt_user, opt_password, opt_db, opt_port, NULL, 0);
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < opt_query_num; i++)
    add_query("insert into test(s1) values(1)");
  ...
  struct event_base *libevent_base = (struct event_base *)event_init();
